...
## Overview
This Python application is designed to analyze Facebook chat data. It extracts, processes, and provides insights into users' messaging patterns and trends. The application supports analysis of single or multiple chats, offering various analytical tools to understand communication dynamics on Facebook.
...
- **Initial Processing:** The `clean_messages` function takes the unified DataFrame and begins the cleaning process. It doesn't directly handle JSON-to-DataFrame conversion but works with the data already in DataFrame format.
...
1. **Message Stats:** This feature aggregates total message counts and words used in conversations. It displays data in three columns showing the user's, the chat partner's (or all partners' in group chats), and the total count.
